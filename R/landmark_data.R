...
#' 
#' @param time Landmark time. Default = 0 (no landmarking).
...
landmark_data <- function(time = 0, mFData_train, X_baseline_train, Y_surv_train, age_train = NULL, mFData_pred = NULL, 
  ...
  X_baseline_train <- X_baseline_train[which_alive_train,]
  ...
  if(!is.null(mFData_pred)){
    ...
    if(!is.null(Y_surv_pred)){
      ...
      X_baseline_pred <- X_baseline_pred[which_alive_pred,]
