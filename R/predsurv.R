...
predsurv <- function(step2, times_pred = step2$landmark_time, 
  ...
  names_base <- names(step1$X_baseline_train)
  names_long <- colnames(step2$traindat)[1:step2$M]
  ...
  #We make a data frame which contains survival and covariates for training data and prediction data
  traindat <- cbind(step2$Y_train, step2$traindat)
  preddat <- cbind(step2$Y_pred, step2$preddat)
  
  
  #Requisites for tune_penCSC() function
  #Create a variable list indicating all occuring competing risks in the data
  comprisknames <- setdiff(unique(step2$Y_train[, "status"]), 0)
  varslist <- vector(mode = "list", length = length(comprisknames))
  names(varslist) <- comprisknames
  alphalist <- vector(mode = "list", length = length(comprisknames))
  names(alphalist) <- comprisknames
  for(i in 1:length(varslist)){
    varslist[[i]] <- as.formula(~.)
    alphalist[[i]] <- c(0)
  }

  if(isTRUE(reg_long) & isTRUE(reg_baseline)){
    keepnames = c(names_long, names_base)
  } else if(isTRUE(reg_long)){
    keepnames = names_long
  } else if(isTRUE(reg)){
    keepnames = names_base
  } else{
    keepnames = NULL
  }
  ...
  #We fit a cause specific 
  cv_fit_train <- tune_penCSC(
    time = "time",
    status = "status",
    vars.list = varslist,
    data = traindat,
    horizons = times_pred,
    event = 1,
    rhs = as.formula(paste("~",paste0(names_base, collapse = "+"))),
    method = 'cv',
    k = 5,
    metrics = "AUC",
    alpha.grid = alphalist,
    keep = keepnames
  )
  asd <<- cv_fit_train
